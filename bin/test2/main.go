...
// This executable successfully create a valid refrenced body + signature combo.
...
func main() {
	...
	err = readXMLDoc.ReadFromBytes(sampleEnv)
	errCheck(err)
	elementToSign := readXMLDoc.FindElement("./Envelope/Body")

	elementToSign.Space = ""
	elementToSign.ChildElements()[0].Space = ""
	elementToSign.SelectAttr("Id").Space = ""

	signedElement, err := signingContext.SignEnveloped(elementToSign)
	errCheck(err)
	readXMLDoc.SetRoot(signedElement)
	b, err := readXMLDoc.WriteToBytes()
	errCheck(err)
...
func (s *Store) GetKeyPair() (privateKey *rsa.PrivateKey, cert []byte, err error) {
	return s.Pk, s.Crt.Raw, nil
}
