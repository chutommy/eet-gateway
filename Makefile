...
.PHONY: test-reports
test-reports: coverage-report unit-test-report e2e-test

.PHONY: coverage-report
coverage-report:
	docker run -t -v $(PWD)/reports:/gen --rm $$(docker build -f tests/coverage_report.Dockerfile -q .)
...
unit-test-report:
	docker run -t -v $(PWD)/reports:/gen --rm $$(docker build -f tests/unit_report.Dockerfile -q .)

.PHONY: e2e-test
e2e-test:
	# https://hub.docker.com/r/postman/newman/
	docker run -t --network="host" -v $(PWD)/certs:/certs postman/newman \
		run "https://www.getpostman.com/collections/b9a63360faf9758ea4fc" \
...
e2e-test-report:
	...
	docker run -t --network="host" -v $(PWD)/reports:/etc/newman -v $(PWD)/certs:/certs dannydainton/htmlextra \
